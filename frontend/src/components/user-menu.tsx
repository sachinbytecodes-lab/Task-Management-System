    ...
    return (
      <div className="relative">
        <button
          ...
          className={`w-9 h-9 rounded-full bg-gradient-to-br ${user?.avatarGradient ?? "from-fuchsia-500 via-purple-500 to-indigo-600"} flex items-center justify-center text-white text-xs font-semibold shrink-0`}
        >
          {(user?.name ?? "D")[0]}
        </button>
        <Dropdown open={open} onClose={() => setOpen(false)} anchorClassName="left-full top-0 ml-2 w-72">
          <UserMenuContent
            user={user}
            sub={sub}
            setSub={setSub}
...
function UserMenuContent({
  ...
  user: any;
  sub: "none" | "theme" | "color";
  setSub: (s: "none" | "theme" | "color") => void;
  mode: string;
  color: ColorMode;
  setMode: (m: any) => void;
